...
export async function GET() {
  const records = await prisma.scope2Record.findMany({ where: { companyId: DEMO_COMPANY_ID }, orderBy: { createdAt: "desc" } });
...
export async function POST(request: Request) {
  ...
  await createAuditEvent({ companyId: DEMO_COMPANY_ID, entityType: "scope2", entityId: record.id, action: "created", actor: "user" });
