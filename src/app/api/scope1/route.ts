...
// GET /api/scope1 - list scope 1 records for demo company (current year)
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch scope 1 records" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to create scope 1 record" }, { status: 500 });
  }
}
