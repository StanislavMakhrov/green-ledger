...
function AddRecordForm({ onAdded }: { onAdded: () => void }) {
  ...
  return (
    ...
    >
      <h2 className="text-base font-semibold mb-4 text-gray-800">Add Scope 1 Record</h2>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            className="border rounded-lg px-3 py-2 text-sm w-full"
            placeholder="e.g. combustion, IPCC AR6"
...
export default function Scope1Page() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await fetch("/api/scope1");
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Scope 1 — Direct Emissions</h1>
      <AddRecordForm onAdded={load} />

      {loading ? (
        <p className="text-gray-400 animate-pulse">Loading…</p>
      ) : records.length === 0 ? (
        <p className="text-gray-400">No Scope 1 records yet.</p>
      ) : (
