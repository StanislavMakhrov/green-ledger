...
export default function Scope2Page() {
  ...
  const load = () =>
    fetch("/api/scope2").then((r) => r.json()).then(setRecords);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Scope 2 — Energy Indirect Emissions</h2>
          <p className="text-sm text-gray-500 mt-1">
            Total: {records.reduce((s, r) => s + r.valueTco2e, 0).toFixed(2)} tCO₂e
          </p>
        </div>
        <button onClick={() => setShowForm(!showForm)} className="bg-green-700 text-white px-4 py-2 rounded-md hover:bg-green-800">
          + Add Record
        </button>
      </div>
